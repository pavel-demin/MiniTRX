...
typedef struct _sender
{
	...
	int arg2;
	int arg3;
	float* out;		// internally created buffer into which processed data is placed
						// a pointer to *out is passed to the external function that is called
} sender, *SENDER;

extern SENDER create_sender (int run, int flag, int mode, int size, float* in, int arg0, int arg1, int arg2, int arg3);
