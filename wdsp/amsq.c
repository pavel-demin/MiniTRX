...
AMSQ create_amsq (int run, int size, float* in, float* out, float* trigger, int rate, float avtau, float tup, float tdown, float tail_thresh, float unmute_thresh, float min_tail, float max_tail, float muted_gain)
{
	...
	// signal averaging
	a->trigger = trigger;
	a->trigsig = (float *)malloc0 (a->size * sizeof (complex));
	a->avtau = avtau;
...
PORT
void SetRXAAMSQThreshold (int channel, float threshold)
{
...
PORT
void SetRXAAMSQMaxTail (int channel, float tail)
{
	AMSQ a;
	EnterCriticalSection (&ch[channel].csDSP);
...
PORT
void SetTXAAMSQMutedGain (int channel, float dBlevel)
{	// dBlevel is negative
	AMSQ a;
	EnterCriticalSection (&ch[channel].csDSP);
...
PORT
void SetTXAAMSQThreshold (int channel, float threshold)
{
	float thresh = pow (10.0, threshold / 20.0);
	EnterCriticalSection (&ch[channel].csDSP);
