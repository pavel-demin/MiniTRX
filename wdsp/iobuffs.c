...
void create_slews (IOB a)
{
	...
	a->slew.cup   = (float *) malloc0 ((a->slew.ntup + 1) * sizeof (float));
	a->slew.cdown = (float *) malloc0 ((a->slew.ntdown + 1) * sizeof (float));

	delta = PI / (float)a->slew.ntup;
	theta = 0.0;
...
void downslew0 (IOB a, float* pout)
{
	int i;
	float *pin;
	float I, Q;
...
void create_iobuffs (int channel)
{
	...
	a->r1_baseptr = (float*) malloc0 (a->r1_active_buffsize * sizeof (complex));
	a->r2_baseptr = (float*) malloc0 (a->r2_active_buffsize * sizeof (complex));
	a->r1_inidx = 0;
	a->r1_outidx = 0;
...
PORT	//float, interleaved I/Q
void fexchange0 (int channel, float* in, float* out, int* error)
{
	int n;
	int doit = 0;
	IOB a;
	...
	*error = 0;
	if (_InterlockedAnd (&ch[channel].exchange, 1))
	{
		...
		if (_InterlockedAnd (&a->slew.upflag, 1))
			...
			for (i = 0; i < a->in_size; i++)
			{
				(a->r1_baseptr + 2 * a->r1_inidx)[2 * i + 0] = (float)(Iin[i]);
				(a->r1_baseptr + 2 * a->r1_inidx)[2 * i + 1] = (float)(Qin[i]);
			}
																										// add check with *error += -1; for case when r1 is full and an overwrite occurs
		if ((a->r1_unqueuedsamps += a->in_size) >= a->r1_outsize)
		{
...
void dexchange (int channel, float* in, float* out)
{
	int n;
