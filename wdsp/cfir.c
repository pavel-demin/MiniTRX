...
CFIR create_cfir (int run, int size, float* in, float* out, int runrate, int cicrate, int DD, int R, int Pairs, float cutoff, int xtype, float xbw)
...
//  xbw:  width of raised cosine transition
{
	...
	a->CFor = fftwf_plan_dft_1d(2 * a->size, (fftwf_complex *)a->infilt,  (fftwf_complex *)a->product, FFTW_FORWARD,  FFTW_ESTIMATE);
	a->CRev = fftwf_plan_dft_1d(2 * a->size, (fftwf_complex *)a->product, (fftwf_complex *)a->out, FFTW_BACKWARD, FFTW_ESTIMATE);
...
void print_response (const char* filename, int N, float* response)
{
	FILE *file;
	int i;
...
float* cfir_impulse (int N, int DD, int R, int Pairs, float runrate, float cicrate, float cutoff, int xtype, float xbw, int rtype, float scale)
{
	...
	float* impulse;
	float* A = (float *) malloc0 (N * sizeof (float));
	...
	float* xistion = (float *) malloc0 ((x_samps + 1) * sizeof (float));
...
float* cfir_mults (int size, int runrate, int cicrate, float scale, int DD, int R, int Pairs, float cutoff, int xtype, float xbw)
{
	// size = input/output size of the FFT-Convolution Filter
	float* impulse;
	float* mults;
	impulse = cfir_impulse (size + 1, DD, R, Pairs, (float)runrate, (float)cicrate, cutoff, xtype, xbw, 1, scale);
