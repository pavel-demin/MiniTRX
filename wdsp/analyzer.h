...
typedef struct _dp
{
	...
	float pix_per_bin;									//number of pixels per fft bin, note that this is fractional, not integral
	float bin_per_pix;									//number of fft bins per pixel, this is fractional and != 1.0/pix_per_bin
	float scale;										//output amplitude scale factor
	float PiAlpha;										//parameter for Kaiser window function

	int cal_set;										//specifies which set of calibration data to use
	float f_min;										//frequency at first pixel (for calibration)
	float f_max;										//frequency at last pixel (for calibration)
	int cal_changed;									//flag to indicate that the calibration data has changed

	float *window;										//pointer to buffer to hold window coefficients	
	float *result[dMAX_STITCH];						//pointers to buffer to hold elimination results for each sub-span
	...
	float *t_pixels;									//pointer to temporary pixel buffer
	float *na_tpixels;									//pointer to temporary pixel buffer for non-averaged data
	...
	float *av_sum;										//pointer to sum buffer for averaging
	float *av_buff[dMAX_AVERAGE];						//pointers to ring of buffers to hold pixel frames for averaging
	float *pre_av_sum;
	float *pre_av_out;
	float *na_out;
	int av_mode;
	float av_backmult;									//back multiplier for weighted averaging
	float *cd;											//pointer to amplitude calibration buffer
	int n_freqs[dMAX_CAL_SETS];							//number of frequencies in each calibration set
	float *freqs[dMAX_CAL_SETS];						//pointers to vectors of calibration frequencies
	float (*ac3[dMAX_CAL_SETS][dMAX_M]);					//pointers to amplitude interpolant coefficients
	float (*ac2[dMAX_CAL_SETS][dMAX_M]);
	float (*ac1[dMAX_CAL_SETS][dMAX_M]);
	float (*ac0[dMAX_CAL_SETS][dMAX_M]);

	fftwf_plan plan[dMAX_STITCH][dMAX_NUM_FFT];			//fftw plans
	fftwf_plan Cplan[dMAX_STITCH][dMAX_NUM_FFT];
	float *fft_in[dMAX_STITCH][dMAX_NUM_FFT];			//pointers to fftw real input vectors
	fftwf_complex *Cfft_in[dMAX_STITCH][dMAX_NUM_FFT];	//pointers to fftw complex input vectors
	fftwf_complex *fft_out[dMAX_STITCH][dMAX_NUM_FFT];	//pointers to fftw complex output vectors
	...
	float *snap_buff[dMAX_STITCH][dMAX_NUM_FFT];		//pointers to buffers for the snap
