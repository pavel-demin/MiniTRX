...
IQC create_iqc (int run, int size, float* in, float* out, float rate, int ints, float tup, int spi)
{
	int i;
	float delta, theta;
	...
	for (i = 0; i < 2; i++)
	{
		a->cm[i] = (float *) malloc0 (a->ints * 4 * sizeof(float));
		a->cc[i] = (float *) malloc0 (a->ints * 4 * sizeof(float));
		a->cs[i] = (float *) malloc0 (a->ints * 4 * sizeof(float));
	}
	a->count = 0;
	a->state = 0;
	a->busy = 0;
...
PORT
void GetTXAiqcValues (int channel, float* cm, float* cc, float* cs)
{
	IQC a;
...
PORT
void SetTXAiqcValues (int channel, float* cm, float* cc, float* cs)
{
	IQC a;
	EnterCriticalSection (&ch[channel].csDSP);
	a = txa[channel].iqc.p0;
...
PORT
void SetTXAiqcSwap (int channel, float* cm, float* cc, float* cs)
{
...
PORT
void SetTXAiqcStart (int channel, float* cm, float* cc, float* cs)
{
	IQC a = txa[channel].iqc.p1;
	EnterCriticalSection (&ch[channel].csDSP);
	a->cset = 0;
