...
void new_window(int disp, int type, int size, float PiAlpha)
{
	...
	case 1:					// blackman-harris window (4 term)
		{
			...
			inv_coherent_gain = (float)size / sum;
			for (i = 0; i < size; i++)
	...
	case 2:					// hann window
		{
			...
			for (i = 0; i < size; i++)
			{
				a->window[i] = 0.5 * (1.0 - cos((float)i * arg0));
				sum += a->window[i];
			}
			inv_coherent_gain = (float)size / sum;
			for (i = 0; i < size; i++)
	...
	case 4:					// hamming window
		{
			...
			for (i = 0; i < size; i++)
			{
				a->window[i] = (0.54 - 0.46 * cos((float)i * arg0));
				sum += a->window[i];
			}
			inv_coherent_gain = (float)size / sum;
			for (i = 0; i < size; i++)
	...
	case 5:					// Kaiser window
		{	arg0 = bessi0(PiAlpha);
			...
			for (i = 0; i < size; ++i)
			{
				a->window[i] = bessi0(PiAlpha * sqrt(1.0 - pow(2.0 * (float)i / arg1 - 1.0, 2))) / arg0;
				sum += a->window[i];
			}
			inv_coherent_gain = (float)size / sum;
			for (i = 0; i < size; i++)
...
void stitch(int disp)
{
	...
	case 5:		// weighted averaging of linear data (low noise floor)
		{
			float onem_avb = 1.0 - a->av_backmult;
			for (n = a->begin_ss; n <= a->end_ss; n++)
	...
	default:
		{
			float *ptr = a->pre_av_out;
			for (n = a->begin_ss; n <= a->end_ss; n++)
			{
				memcpy(ptr, a->result[n], a->ss_bins[n] * sizeof(float));
				ptr += a->ss_bins[n];
				m += a->ss_bins[n];
			}
			break;
	...
	if (a->pix_per_bin <= 1.0)
	{
		for (i = 0; i < m; i++)
		{
			pix_count = (int)((float)i * a->pix_per_bin);
	...
	else
	{
		...
		for (i = 1; i < m; i++)
		{
			while (pix_pos < (float)i)
			{
				frac = pix_pos - (float)(i - 1);
	...
	case 3:		// window averaging of linear data
		{
			...
			{
				factor = a->scale / (float)++a->avail_frames;
				for (i = 0; i < a->num_pixels; i++)
				{
					a->av_sum[i] += a->t_pixels[i];
	...
	case 4:		// window averaging of log data
		{
			if (a->avail_frames < a->num_average)
			{
				factor = 1.0 / (float)++a->avail_frames;
				for (i = 0; i < a->num_pixels; i++)
				{
...
DWORD WINAPI spectra (void *pargs)
{
	...
	if ((ss >= a->begin_ss) && (ss <= a->end_ss))
	{
		for (i = 0; i < a->size; i++)
		{
			(a->fft_in[ss][LO])[i] = a->window[i] * (float)((a->I_samples[ss][LO])[a->IQO_idx[ss][LO]]);
...
DWORD WINAPI Cspectra (void *pargs)
{
	...
	int LO = ((int)pargs) & 15;
	DP a = pdisp[disp];
	int trans_size = a->size * sizeof(float);
	...
	if ((ss >= a->begin_ss) && (ss <= a->end_ss))
	{
		for (i = 0; i < a->size; i++)
		{
			(a->Cfft_in[ss][LO])[i][0] = a->window[i] * (float)((a->I_samples[ss][LO])[a->IQO_idx[ss][LO]]);
			(a->Cfft_in[ss][LO])[i][1] = a->window[i] * (float)((a->Q_samples[ss][LO])[a->IQO_idx[ss][LO]]);
			if(++a->IQO_idx[ss][LO] >= a->bsize)
				 a->IQO_idx[ss][LO] -= a->bsize;
		}
...
void interpolate(int disp, int set, float fmin, float fmax, int num_pixels)
{
	DP a = pdisp[disp];
	int i; 
	float f;
    ...
    {
		f = fmin + (float)i * (fmax - fmin) / (float)(num_pixels - 1);
...
int build_interpolants(int disp, int set, int n, int m, float *x, float (*y)[dMAX_M])
{
	DP a = pdisp[disp];
	float dx[dMAX_N];
	float idx[dMAX_N];
	...
	float dsub[dMAX_N];
	float dsup[dMAX_N];
	float d[dMAX_N][dMAX_M];
	float S[dMAX_N][dMAX_M];
	float b[dMAX_N];
	float v[dMAX_N][dMAX_M];
	float tmp;
	int i, j;
...
void SetAnalyzer (	int disp,
					...
					float pi,			//PiAlpha parameter for Kaiser window
					...
					float av_b,		//back multiplier for weighted averaging
					int calset,			//identifier of which set of calibration data to use 
					float fmin,		//frequency at first pixel value 
					float fmax,		//frequency at last pixel value
					int max_w
				 )
{
	DP a = pdisp[disp];
	...
	if (av_m != a->av_mode)
	{
		...
		default:
			{
				memset ((void *)a->av_sum, 0, sizeof(float) * dMAX_PIXELS);
				break;
			}
		}
	}
	...
	if (sz != a->size)
	{
		for (i = 0; i < a->max_stitch; i++)
			...
			{
				if (a->plan[i][j])		fftwf_destroy_plan (a->plan[i][j]);
				if (a->Cplan[i][j])		fftwf_destroy_plan (a->Cplan[i][j]);
				a->plan[i][j] = fftwf_plan_dft_r2c_1d(sz, a->fft_in[i][j], a->fft_out[i][j], FFTW_PATIENT);
				a->Cplan[i][j] = fftwf_plan_dft_1d(sz, a->Cfft_in[i][j], a->fft_out[i][j], FFTW_FORWARD, FFTW_PATIENT);
			}
	}
	...
	if (a->type == 0)
	{
		...
		a->scale = 4.0 / ((float)a->size * (float)a->size);
	}
	else
	{
		a->out_size = a->size;
		a->scale = 1.0 / ((float)a->size * (float)a->size);
	}
	...
	a->pix_per_bin = (float)a->num_pixels / (float)(a->num_stitch * (a->out_size - 2 * a->clip) - a->fsclipL - a->fsclipH);
	a->bin_per_pix = (float)(a->num_stitch * (a->out_size - 2 * a->clip) - 1 - a->fsclipL - a->fsclipH) / (float)a->num_pixels;
						...
						)
{
	...
	*success = 0;
	...
	a->window = (float*) malloc0 (sizeof(float) * a->max_size);
	if (a->window == 0)
		*success += -1;
	for (i = 0; i < a->max_stitch; i++)
	{
		a->result[i] = (float*) malloc0 (sizeof(float) * a->max_size);
		if (a->result[i] == 0)
			flag = 1;
	}
	...
	for (i = 0; i < a->max_stitch; i++)
		for (j = 0; j < a->max_num_fft; j++)
		{
			a->plan[i][j] = 0;
			a->Cplan[i][j] = 0;
			a->fft_in[i][j]   = (float*) malloc0 (sizeof(float) * a->max_size);
			a->Cfft_in[i][j]  = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * a->max_size);
			a->fft_out[i][j]  = (fftwf_complex*) fftwf_malloc(sizeof(fftwf_complex) * a->max_size);
			if ((a->fft_in[i][j] == 0) || (a->fft_out[i][j] == 0) || (a->Cfft_in[i][j] == 0))
				flag = 1;
		}
	if (flag) *success += -4, flag = 0;
	a->pre_av_sum = (float*) malloc0 (sizeof(float) * a->max_size * a->max_stitch);
	a->pre_av_out = (float*) malloc0 (sizeof(float) * a->max_size * a->max_stitch);
	a->na_out     = (float*) malloc0 (sizeof(float) * a->max_size * a->max_stitch);
	a->av_sum     = (float*) malloc0 (sizeof(float) * dMAX_PIXELS);
	if ((a->av_sum == 0) || (a->pre_av_sum == 0) || (a->pre_av_out == 0) || (a->na_out == 0))
		*success += -8;

	for (i = 0; i < dMAX_AVERAGE; i++)
	{
		a->av_buff[i] = (float*) malloc0 (sizeof(float) * dMAX_PIXELS);
		if (a->av_buff[i] == 0)
			flag = 1;
	}
	...
	a->cd = (float*) malloc0 (sizeof(float) * dMAX_PIXELS);
		if (a->cd == 0)
			*success += -64;
		else
	...
	{
		a->freqs[i] = (float*) malloc0 (sizeof(float) * dMAX_N);
		for (j = 0; j < dMAX_M; j++)
		{
			a->ac3[i][j] = (float*) malloc0 (sizeof(float) * dMAX_N);
			a->ac2[i][j] = (float*) malloc0 (sizeof(float) * dMAX_N);
			a->ac1[i][j] = (float*) malloc0 (sizeof(float) * dMAX_N);
			a->ac0[i][j] = (float*) malloc0 (sizeof(float) * dMAX_N);
			if((a->freqs[i] == 0) || (a->ac3[i][j] == 0) || (a->ac2[i][j] == 0) || (a->ac1[i][j] == 0) || (a->ac0[i][j] == 0))
				flag = 1;
		}
	}
	if (flag) *success += -128, flag = 0;
	a->t_pixels   = (float*) malloc0 (sizeof(float) * dMAX_PIXELS);
	a->na_tpixels = (float*) malloc0 (sizeof(float) * dMAX_PIXELS);
...
void DestroyAnalyzer(int disp)
{
	...
	for (i = 0; i < a->max_stitch; i++)
		for (j = 0; j < a->max_num_fft; j++)
		{
			fftwf_destroy_plan (a->plan[i][j]);
			fftwf_destroy_plan (a->Cplan[i][j]);
			fftwf_free (a->Cfft_in[i][j]);
			_aligned_free (a->fft_in[i][j]);
			fftwf_free (a->fft_out[i][j]);
		}
...
void SetCalibration (	int disp,
						...
						float (*cal)[dMAX_M+1]		//pointer to the calibration table, first
													...
													//	 data for variables being calibrated
{
	...
	int i, j;
	int k = 0;
	float y [dMAX_N][dMAX_M];

	qsort (cal, n_points, (dMAX_M+1) * sizeof(float), calcompare);
...
PORT
void Spectrum0(int run, int disp, int ss, int LO, float* pbuff)
{
	if (run)
	{
		int i;
