	...
	int rate, int wintype, float gain, int gain_method, int npe_method, int ae_run)
{
	...
	a->rate = rate;
	a->wintype = wintype;
	a->gain = gain / fsize / (float)a->ovrlp;
	if (a->fsize > a->bsize)
		a->iasize = a->fsize;
	else
	...
	a->g.gmax = 10000.0;
	//
	a->g.GG = (float *)malloc0(241 * 241 * sizeof(float));
	a->g.GGS = (float *)malloc0(241 * 241 * sizeof(float));
	a->g.fileb = fopen("calculus", "rb");
	fread(a->g.GG, sizeof(float), 241 * 241, a->g.fileb);
	fread(a->g.GGS, sizeof(float), 241 * 241, a->g.fileb);
	fclose(a->g.fileb);
	//
...
float getKey(float* type, float gamma, float xi)
{
