...
PORT
void OpenChannel (int channel, int in_size, int dsp_size, int input_samplerate, int dsp_rate, int output_samplerate, int type, int state, float tdelayup, float tslewup, float tdelaydown, float tslewdown, int bfo)
{
...
PORT
void SetChannelTDelayUp (int channel, float time)
{
	IOB a;
	EnterCriticalSection (&ch[channel].csEXCH);
	a = ch[channel].iob.pc;
...
PORT
void SetChannelTSlewUp (int channel, float time)
{
	IOB a;
	EnterCriticalSection (&ch[channel].csEXCH);
	a = ch[channel].iob.pc;
...
PORT
void SetChannelTDelayDown (int channel, float time)
{
	IOB a;
	EnterCriticalSection (&ch[channel].csEXCH);
	a = ch[channel].iob.pc;
...
PORT
void SetChannelTSlewDown (int channel, float time)
{
	IOB a;
	EnterCriticalSection (&ch[channel].csEXCH);
	a = ch[channel].iob.pc;
