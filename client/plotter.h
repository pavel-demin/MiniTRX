...
public:
  ...
  void SetSpanFreq(quint32 s){m_Span=(int)s;}
...
signals:
  void NewCenterFreq(int f);
  void NewDemodFreq(int f);
...
private:
  ...
  int XfromFreq(int freq);
  int FreqfromX(int x);
  int RoundFreq(int freq, int resolution);
  bool IsPointCloseTo(int x, int xr, int delta){return ((x > (xr-delta)) && ( x<(xr+delta)));}
  void ClampDemodParameters();
  void DisplayCursorFreq(QPoint pt, int freq);
