	...
	)
{
	...
	a->awave = (float *)malloc0 ((int)(MAX_ADV_SLEW_TIME  * MAX_SAMPLERATE + 1) * sizeof (float));
	a->hwave = (float *)malloc0 ((int)(MAX_HANG_SLEW_TIME * MAX_SAMPLERATE + 1) * sizeof (float));

	a->filterlen = 10;
	a->bfbuff = (float *)malloc0 (a->filterlen * sizeof (complex));
	a->ffbuff = (float *)malloc0 (a->filterlen * sizeof (complex));
	a->fcoefs = (float *)malloc0 (a->filterlen * sizeof (float));
...
void pSetRCVRNOBTau (NOB a, float tau)
{
	a->advslewtime = tau;
	a->hangslewtime = tau;
	init_nob (a);
...
void pSetRCVRNOBHangtime (NOB a, float time)
{
	a->hangtime = time;
	init_nob (a);
}

void pSetRCVRNOBAdvtime (NOB a, float time)
{
	a->advtime = time;
	init_nob (a);
}

void pSetRCVRNOBBacktau (NOB a, float tau)
{
	a->backtau = tau;
	init_nob (a);
}

void pSetRCVRNOBThreshold (NOB a, float thresh)
{
	a->threshold = thresh;
}
...
{
	float advslewtime = slewtime;
	float hangslewtime = slewtime;
	float max_imp_seq_time = 0.025;
...
void xnobEXT (int id, float* in, float* out)
{
	NOB a = pnob[id];
	a->in = in;
	a->out = out;
...
void SetEXTNOBTau (int id, float tau)
{
	NOB a = pnob[id];
	a->advslewtime = tau;
	a->hangslewtime = tau;
...
void SetEXTNOBHangtime (int id, float time)
{
	NOB a = pnob[id];
	a->hangtime = time;
	init_nob (a);
...
void SetEXTNOBAdvtime (int id, float time)
{
	NOB a = pnob[id];
	a->advtime = time;
	init_nob (a);
...
void SetEXTNOBBacktau (int id, float tau)
{
	NOB a = pnob[id];
	a->backtau = tau;
	init_nob (a);
...
void SetEXTNOBThreshold (int id, float thresh)
{
	NOB a = pnob[id];
	a->threshold = thresh;
}
