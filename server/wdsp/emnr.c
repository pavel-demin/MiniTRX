...
void interpM (float* res, float x, int nvals, float* xvals, float* yvals)
{
	if (x <= xvals[0])
		*res = yvals[0];
...
EMNR create_emnr (int run, int position, int size, float* in, float* out, int fsize, int ovrlp, 
	int rate, int wintype, float gain, int gain_method, int npe_method, int ae_run)
{
	...
	a->nsamps   = 0;
	a->saveidx  = 0;
	a->Rfor = fftwf_plan_dft_r2c_1d(a->fsize, a->forfftin, (fftwf_complex *)a->forfftout, FFTW_PATIENT);
    a->Rrev = fftwf_plan_dft_c2r_1d(a->fsize, (fftwf_complex *)a->revfftin, a->revfftout, FFTW_PATIENT);
	calc_window (a);

	a->g.msize = a->msize;
	a->g.mask = a->mask;
	a->g.y = a->forfftout;
	a->g.lambda_y   = (float *) malloc0 (a->msize * sizeof (float));
	a->g.lambda_d   = (float *) malloc0 (a->msize * sizeof (float));
	a->g.prev_gamma = (float *) malloc0 (a->msize * sizeof (float));
	a->g.prev_mask  = (float *) malloc0 (a->msize * sizeof (float));
	...
	a->np.p           = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.alphaOptHat = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.alphaHat    = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.sigma2N     = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.pbar        = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.p2bar       = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.Qeq         = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.bmin        = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.bmin_sub    = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.k_mod       = (int *)    malloc0 (a->np.msize * sizeof (int));
	a->np.actmin      = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.actmin_sub  = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.lmin_flag   = (int *)    malloc0 (a->np.msize * sizeof (int));
	a->np.pmin_u      = (float *) malloc0 (a->np.msize * sizeof (float));
	a->np.actminbuff  = (float**) malloc0 (a->np.U     * sizeof (float*));
	for (i = 0; i < a->np.U; i++)
		a->np.actminbuff[i] = (float *) malloc0 (a->np.msize * sizeof (float));
	...
	a->nps.sigma2N   = (float *) malloc0 (a->nps.msize * sizeof (float));
	a->nps.PH1y      = (float *) malloc0 (a->nps.msize * sizeof (float));
	a->nps.Pbar      = (float *) malloc0 (a->nps.msize * sizeof (float));
	a->nps.EN2y      = (float *) malloc0 (a->nps.msize * sizeof (float));
	...
	a->ae.nmask = (float *) malloc0 (a->ae.msize * sizeof (float));
	return a;
}
...
void destroy_emnr (EMNR a)
{
	...
	for (i = 0; i < a->np.U; i++)
		free (a->np.actminbuff[i]);
	free (a->np.actminbuff);
	free (a->np.pmin_u);
	free (a->np.lmin_flag);
	free (a->np.actmin_sub);
	free (a->np.actmin);
	free (a->np.k_mod);
	free (a->np.bmin_sub);
	free (a->np.bmin);
