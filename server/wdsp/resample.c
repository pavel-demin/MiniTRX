...
RESAMPLE create_resample ( int run, int size, float* in, float* out, int in_rate, int out_rate, float fc, int ncoef, float gain)
{
	...
	a->cpp = a->ncoef / a->L;
	a->h = (float *) malloc0 (a->ncoef * sizeof (float));
	impulse = fir_bandpass (a->ncoef, -fc_norm, +fc_norm, 1.0, 1, 0, a->gain * (float)a->L);
	i = 0;
	for (j = 0; j < a->L; j++)
		for (k = 0; k < a->ncoef; k += a->L)
			a->h[i++] = impulse[j + k];
	a->ringsize = a->cpp;
	a->ring = (float *) malloc0 (a->ringsize * sizeof (complex));
...
void xresampleV (float* input, float* output, int numsamps, int* outsamps, void* ptr)
{
...
RESAMPLEF create_resampleF ( int run, int size, float* in, float* out, int in_rate, int out_rate)
{
	...
	a->cpp = a->ncoef / a->L;
	a->h = (float *) malloc0 (a->ncoef * sizeof (float));
	impulse = fir_bandpass (a->ncoef, -fc_norm, +fc_norm, 1.0, 1, 0, (float)a->L);
	i = 0;
	for (j = 0; j < a->L; j ++)
		for (k = 0; k < a->ncoef; k += a->L)
			a->h[i++] = impulse[j + k];
	a->ringsize = a->cpp;
	a->ring = (float *) malloc0 (a->ringsize * sizeof (float));
	a->idx_in = a->ringsize - 1;
	a->phnum = 0;
	free (impulse);
	return a;
